...
public class InstancesServiceJPA implements ExtendedInstancesService {
	...
	@Autowired
	public InstancesServiceJPA(InstanceRepository instanceRepository, InstanceConverter instanceConverter, ConfigProperties configProperties, ExtendedUsersService userService) {
	...
	private InstanceEntity getInstanceEntityById(String instanceDomain, String instanceId) {
		Optional<InstanceEntity> op = instanceRepository.findById(instanceConverter.toEntity(instanceDomain, instanceId));
	...
	public List<InstanceBoundary> getAllInstances(String userDomain, String userEmail, int size, int page) {
		...
		case MANAGER:
			return instanceRepository
					.findAll(PageRequest.of(page, size, Direction.ASC, "instanceId"))
					.stream() 
